 ...
 * @since   1.0.0
 */
namespace Bmd\WPFramework\Scoper;
...
class Package_Twig_Prefixer
{
     ...
     * @return string
     */
    public function getFormattedPrefix(int $backslashDuplicationFactor = 0, bool $includeInitialBackslash = \true): string
    {
    ...
    public function patchForAll(string $contents): string
    {
        ...
        $contents = preg_replace('/("|\')((\\\\){1,2}Twig(\\\\){1,2}[A-Za-z\\\\]+)\1/m', '$1' . $this->getFormattedPrefix(2) . '$2$1', $contents);
        // Hardcoded "use" statements
        $contents = preg_replace('/use\s+(Twig)(\\\\){1,2}/m', 'use ' . $this->getFormattedPrefix(2) . '\\\\\\\\Twig\\\\\\\\', $contents);
        // Addnamespaces to generated Twig template names
        $contents = preg_replace('/(\'|")(__TwigTemplate_)\1/m', '$1' . $this->getFormattedPrefix(2) . '\\\\\\\\$2$1', $contents);
        return $contents;
    }
    /**
    ...
    {
        if (!$this->endsWith($filePath, 'src' . \DIRECTORY_SEPARATOR . 'Node' . \DIRECTORY_SEPARATOR . 'ModuleNode.php')) {
            return $contents;
        }
        ...
        $contents = preg_replace('/(compileClassHeader\s*\([^\)]+\)\s*{\s*\s*\$compiler\s*->\s*write\s*\(\s*)"\\\\n\\\\n"(\s*\)\s*;)/m', '$1"\n\nnamespace ' . $this->getFormattedPrefix(2, \false) . ';\n\n"$2', $contents);
        ...
        $contents = preg_replace('/(->write\s*\(\s*\'class \'\s*\.\s*)(\$compiler\s*->\s*getEnvironment\s*\(\s*\)\s*->\s*getTemplateClass\s*\(\s*\$this\s*->\s*getSourceContext\s*\(\s*\)\s*->\s*getName\s*\(\s*\)\s*,\s*\$this\s*->\s*getAttribute\s*\(\s*\'index\'\s*\)\s*\))/m', '$1 \substr( $2, ' . strlen($string_to_remove) . ' ) ', $contents);
        return $contents;
    }
    /**
    ...
    public function patchForCoreExtension(string $contents, string $filePath): string
    {
        ...
        if (!$this->endsWith($filePath, 'src' . \DIRECTORY_SEPARATOR . 'Extension' . \DIRECTORY_SEPARATOR . 'CoreExtension.php')) {
            return $contents;
        }
        $contents = preg_replace('/(TwigFilter\(\s*\'[^\']+\'\s*,\s*\')((_)?twig_[^\']+\')/m', '$1' . $this->getFormattedPrefix(2) . '\\\\\\\\$2', $contents);
        // Handle the occurrence in the is_safe_callback array element.
        $contents = preg_replace('/(new ' . $this->getFormattedPrefix(1) . '\\\\Twig\\\\TwigFilter\(\s*\'[^\']+\'\s*,\s*\'.*twig_[^\']+\',\s*\[[^\]]*,\s*\'is_safe_callback\'\s*=>\s*\')((_)?twig_[^\']+\'\s*\]\s*\))/m', '$1' . $this->getFormattedPrefix(2) . '\\\\\\\\$2', $contents);
        // Handle the occurrence in the preg_replace_callback.
        $contents = preg_replace('/(preg_replace_callback.*,\s*\')(.+\'.*;)/m', '$1' . $this->getFormattedPrefix(2) . '\\\\\\\\$2', $contents);
        // Handle the occurrence in the array_walk_recursive.
        $contents = preg_replace('/(array_walk_recursive.*,\s*\')(.+\'.*;)/m', '$1' . $this->getFormattedPrefix(2) . '\\\\\\\\$2', $contents);
    ...
    public function patchForEnvironment(string $contents, string $filePath): string
    {
        ...
        $contents = preg_replace('/(Twig\\\\.+\\\\AbstractExtension)/m', $this->getFormattedPrefix(2, \false) . '\\\\\\\\$1', $contents);
        return $contents;
    }
    /**
     ...
     * template (namespace Bmd\WPFramework\Deps), this causes the class to be resolved relative
    ...
    {
        if (!$this->endsWith($filePath, 'src' . \DIRECTORY_SEPARATOR . 'Node' . \DIRECTORY_SEPARATOR . 'Expression' . \DIRECTORY_SEPARATOR . 'CallExpression.php')) {
            return $contents;
        }
        ...
        $contents = str_replace('$compiler->raw(\sprintf(\'%s::%s\', $callable[0], $callable[1]));', '$compiler->raw(\sprintf(\'\\\\%s::%s\', \ltrim($callable[0], \'\\\\\'), $callable[1]));', $contents);
        return $contents;
    }
    /**
    ...
    {
        if (!$this->endsWith($filePath, 'src' . \DIRECTORY_SEPARATOR . 'Node' . \DIRECTORY_SEPARATOR . 'CaptureNode.php')) {
            return $contents;
        }
        $contents = str_replace("'\\\\Twig\\\\Extension\\\\CoreExtension::captureOutput('", "'\\\\" . $this->getFormattedPrefix(1, \false) . "\\\\\\\\Twig\\\\\\\\Extension\\\\\\\\CoreExtension::captureOutput('", $contents);
        return $contents;
    }
}
