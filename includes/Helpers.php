...
class Helpers
{
	 ...
	 *
	 * @param mixed $array Array to check for list format. This parameter accepts any value but should be an array.
	 *
	 * @return boolean
	 */
	...
	public static function truthyFalsy( $value ): bool
	{
		switch ( \true ) {
			...
			case is_string( $value ):
				return in_array( strtolower( $value ), [ 'true', 'yes', 'on', '1' ], \true );
	...
	/**
	 * Replace slashes, spaces, hyphens, and camel‑case transitions
	 * with underscores, then convert to lowercase.
	 *
	...
	{
		/**
		 * 1. Replace slashes, spaces, hyphens, and underscores with underscores.
		 *    This also replaces any leading or trailing whitespace.
		 */
		$string = str_replace( [ '\\', '/', ' ', '-' ], '_', $raw_string );
		/**
		 * 2. Split camelCase or PascalCase: insert underscores
		 * between a lowercase/number and uppercase.
		 */
		$string = preg_replace( '/([a-z0-9])([A-Z])/', '$1_$2', $string );
		/**
		 * 3. Remove any non-word characters (except underscores).
		 *    This also removes any leading or trailing underscores.
		 */
	 ...
	 *
	 * @param string $raw_string string that should be hyphenated.
	 *
	 * @return string
	 */
	 ...
	 * Merge two arrays
	 *
	 * @param array<mixed> $array1 : first array to merge.
	 * @param array<mixed> $array2 : second array to merge.
	 *
	 * @return array<mixed>
	 */
