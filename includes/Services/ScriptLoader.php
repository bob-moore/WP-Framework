	...
	*/
	public function scriptData(
		...
		*/
		...
		return [
			'handle'       => Helpers::hyphenate( $handle ),
			'src'          => $src,
			'dependencies' => apply_filters( "{$handle}_script_dependencies", $dependencies ),
			'version'      => $version,
			'in_footer'    => $in_footer,
		];
	}
	 ...
	 * @param string             $src : relative path to script.
	 * @param array<int, string> $dependencies : any set dependencies not in assets file, optional.
	 * @param string\null        $version : version of JS file, optional.
	 * @param boolean            $in_footer : whether to enqueue in footer, optional.
	 *
	 * @return void
	 */
